...
class StockComparatifReport(models.Model):
    _name = "opsol_costadoro.stock_comparatif_report"
    _description = "Stock Comparatif Report"
    ...
    estimation = fields.Float("Estimation", readonly=True)
    date_year = fields.Integer("Annee", readonly=True)
    product_qty = fields.Float("Livré", readonly=True)
    est_diff = fields.Float("Difference", readonly=True)
    # is_late = fields.Boolean("Is Late", readonly=True)
    # company_id = fields.Many2one('res.company', 'Company', readonly=True)
    # categ_id = fields.Many2one('product.category', 'Product Category', readonly=True)

    @api.depends('date_year', 'partner_id.name')
    def _compute_display_name(self):
        for report in self:
            report.display_name = f'{report.date_year} - {report.partner_id.display_name}'
    ...
    def _select(self):
        select_str = """
            ...
            -- cat.id as categ_id
            sp.partner_id as partner_id,
            resp.estimation_qty as estimation,
            date_part('year', sp.scheduled_date) as date_year,
            SUM(sm.quantity) as product_qty,
            resp.estimation_qty - SUM(sm.quantity * sm.out_move_sign) as est_diff
        """
    ...
    def _from(self):
        from_str = """
            ...
            INNER JOIN res_partner resp ON sp.partner_id = resp.id
            WHERE sm.out_move_sign != 0 AND sp.partner_id IS NOT NULL
        """
    ...
    def _group_by(self):
        group_by_str = """
            ...
            sp.partner_id,
            resp.estimation_qty,
            date_part('year', sp.scheduled_date)
        """
