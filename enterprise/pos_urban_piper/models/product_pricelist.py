...
class ProductPricelist(models.Model):
    ...
    def write(self, vals):
        ...
        for pricelist in self:
            urban_piper_configs = self.env['pos.config'].search([
                ('company_id', '=', pricelist.company_id.id),
                ('module_pos_urban_piper', '=', True),
                ('urbanpiper_pricelist_id', '=', pricelist.id),
            ])
            if urban_piper_configs:
                linked_urban_piper_status = self.env['product.template'].search([('urbanpiper_pos_config_ids', 'in', urban_piper_configs.ids)])\
